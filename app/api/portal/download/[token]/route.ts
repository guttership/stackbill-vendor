...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  const consumed = await consumeDownloadToken(token)

  if (!consumed) {
    return NextResponse.json({ error: 'Invalid or expired token' }, { status: 400 })
  }

  const license = await findLicenseById(consumed.licenseId)
  if (!license || !isLicenseActive(license)) {
    return NextResponse.json({ error: 'License inactive' }, { status: 403 })
  }

  const artifact = await findArtifactById(consumed.artifactId)
  ...
  await logDownloadEvent(license.id, artifact.id, ip, userAgent)
